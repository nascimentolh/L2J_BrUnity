...
import org.l2jbr_unity.gameserver.model.actor.instance.OlympiadManager;
import org.l2jbr_unity.gameserver.model.holders.SkillHolder;
import org.l2jbr_unity.gameserver.model.olympiad.CompetitionType;
import org.l2jbr_unity.gameserver.model.olympiad.Hero;
import org.l2jbr_unity.gameserver.model.olympiad.Olympiad;
import org.l2jbr_unity.gameserver.model.skill.Skill;
import org.l2jbr_unity.gameserver.network.serverpackets.ExHeroList;
		...
		try
		{
			...
			else if (command.toLowerCase().startsWith("olympiadnoble"))
			{
				...
				switch (val)
				{
					...
					{
						if (!org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().isRegistered(player))
						{
							...
							html.replace("%olympiad_opponent%", String.valueOf(org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().getCountOpponents()));
							player.sendPacket(html);
						}
						else
						{
					...
					case 1: // unregister
					{
						org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().unRegisterNoble(player);
						break;
					}
					case 2: // show waiting list | TODO: cleanup (not used anymore)
					{
						final int nonClassed = org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().getRegisteredNonClassBased().size();
						final int teams = org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().getRegisteredTeamsBased().size();
						final Collection<List<Integer>> allClassed = org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().getRegisteredClassBased().values();
						int classed = 0;
					...
					{
						org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().registerNoble(player, CompetitionType.NON_CLASSED);
						break;
					}
					case 5: // register classed
					{
						org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().registerNoble(player, CompetitionType.CLASSED);
						break;
					}
					...
					{
						org.l2jbr_unity.gameserver.model.olympiad.OlympiadManager.getInstance().registerNoble(player, CompetitionType.TEAMS);
						break;
					}
					default:
					{
